...
void HandleAction_UseMove(void)
{
        ...
        && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gSideTimers[side].followmeTarget))
    {
        ...
        if(GetBattlerAbility(gBattlerTarget) == ABILITY_GUARD_DOG){
...
void PrepareStringBattle(u16 stringId, u32 battler)
{
    ...
    else if (stringId == STRINGID_STATSWONTDECREASE2 && (battlerAbility == ABILITY_CONTRARY || battlerAbility == ABILITY_DOUBLE_DOWN))
        stringId = STRINGID_STATSWONTINCREASE2;
    else if (stringId == STRINGID_STATSWONTINCREASE2 && (battlerAbility == ABILITY_CONTRARY || battlerAbility == ABILITY_DOUBLE_DOWN))
    ...
    else if ((stringId == STRINGID_PKMNCUTSATTACKWITH || stringId == STRINGID_PKMNCUTSSPECIALATTACKWITH) && targetAbility == ABILITY_RATTLED
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
    ...
    if (DYNAMAX_BYPASS_CHECK && (GetBattlerAbility(battler) == ABILITY_GORILLA_TACTICS || GetBattlerAbility(battler) == ABILITY_MONKEY_BUSINESS) && *choicedMove != MOVE_NONE
...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Gorilla Tactics
            ...
            unusableMoves |= gBitTable[i];
        else if (check & MOVE_LIMITATION_CHOICE_ITEM && GetBattlerAbility(battler) == ABILITY_MONKEY_BUSINESS && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battler].moves[i])
            unusableMoves |= gBitTable[i];
...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_AQUA_RING:  // aqua ring
            ...
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_ICE_RINK:  // ice rink
            if ((gStatuses4[battler] & STATUS4_ICE_RINK)
             && !BATTLER_MAX_HP(battler)
             && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)
             && gBattleMons[battler].hp != 0)
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 8);
                BattleScriptExecute(BattleScript_IceRinkHeal);
                effect++;
            }
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_QUARRY:  // quarry
            if ((gStatuses4[battler] & STATUS4_QUARRY)
             && !BATTLER_MAX_HP(battler)
             && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)
             && gBattleMons[battler].hp != 0)
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 8);
                BattleScriptExecute(BattleScript_QuarryHeal);
                effect++;
            }
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_SMELTERY:  // smeltery
            if ((gStatuses4[battler] & STATUS4_SMELTERY)
             && !BATTLER_MAX_HP(battler)
             && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)
             && gBattleMons[battler].hp != 0)
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 8);
                BattleScriptExecute(BattleScript_SmelteryHeal);
                effect++;
            }
            gBattleStruct->turnEffectsTracker++;
            break;
        ...
        case ENDTURN_BURN:  // burn
                ...
                && gBattleMons[battler].hp != 0)
            {
                ...
                {
                    if (gBattleMoveDamage > (gBattleMoveDamage / 4) + 1) // Record ability if the burn takes less damage than it normally would.
        ...
        case CANCELLER_ASLEEP: // check being asleep
            if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
            {
                ...
                else
                {
                    ...
                    if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
                    {
                        if (gChosenMove != MOVE_SNORE && gChosenMove != MOVE_SLEEP_TALK && gChosenMove != MOVE_DREAM_EATER)
                        {
...
{
    return UQ_4_12(1.0) + (UQ_4_12(0.15) * gBattleStruct->supremeOverlordCounter[battler]);
}
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_AS_ONE_SHADOW_RIDER:
            ...
            break;
        case ABILITY_BIG_BALLS:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_BIGBALLS;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_ActivateBigBalls);
                effect++;
            }
            break;
        case ABILITY_CATS_CRADLE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_CATSCRADLE;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
        ...
        case ABILITY_ONSLAUGHT_CORE:
             ...
             && !gSpecialStatuses[battler].dancerUsedMove)
            {
                ...
                gBattleStruct->atkCancellerTracker = 0;
                gBattlerAttacker = gBattlerAbility = battler;
                gCalledMove = MOVE_ONSLAUGHT_CORE;
        ...
        case ABILITY_SUPREME_OVERLORD:
            ...
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattleStruct->supremeOverlordCounter[battler] = min(3, GetBattlerSideFaintCounter(battler));
        ...
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_HEALER:
                ...
                if (IsDoubleBattle() && gBattleMons[partner].hp < gBattleMons[partner].maxHP && IsBattlerAlive(partner))
                {
                    gBattlerTarget = partner;
                    gBattlerAttacker = battler;
                    gBattleMoveDamage = (GetNonDynamaxMaxHP(partner) / 16) * -1;
                    BattleScriptPushCursorAndCallback(BattleScript_HealerRestoreActivates);
                    effect++;
                }
                break;
            case ABILITY_SCHOOLING:
            ...
            case ABILITY_COLOR_CHANGE:
                gStatuses4[battler] &= ~STATUS4_COLOR_CHANGE;
                gBattleStruct->turnEffectsTracker++;
                break;
            }
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
             ...
             || (gLastUsedAbility == ABILITY_BULLETPROOF && gBattleMoves[move].ballisticMove))
            {
                ...
                effect = 1;
                  ...
                  && (battlerAbility == ABILITY_DAZZLING || battlerAbility == ABILITY_QUEENLY_MAJESTY || battlerAbility == ABILITY_ARMOR_TAIL || battlerAbility == ABILITY_TIME_LORD)
                  && GetChosenMovePriority(gBattlerAttacker) > 0
        ...
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_WIND_RIDER:
                if (gBattleMoves[gCurrentMove].windMove && !(GetBattlerMoveTargetType(gBattlerAttacker, gCurrentMove) & MOVE_TARGET_USER))
                    effect = 2, statId = STAT_ATK, statAmount = 2;
                break;
            case ABILITY_EARTH_EATER:
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_ANGER_SHELL:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && (gMultiHitCounter == 0 || gMultiHitCounter == 1) // Activates after all hits from a multi-hit move.
             && IsBattlerAlive(gBattlerTarget)
            // && HadMoreThanHalfHpNowHasLess(gBattlerTarget) hopefully this works
             && !(TestSheerForceFlag(gBattlerAttacker, gCurrentMove)))
            {
                gBattlerAttacker = gBattlerTarget;
                BattleScriptPushCursor();
...
static u8 RandomStatRaiseBerry(u32 battler, u32 itemId, bool32 end2)
{
    ...
    if (i != NUM_STATS - 1 && HasEnoughHpToEatBerry(battler, GetBattlerItemHoldEffectParam(battler, itemId), itemId))
    {
        ...
        stringId = (battlerAbility == ABILITY_CONTRARY || battlerAbility == ABILITY_DOUBLE_DOWN) ? STRINGID_STATFELL : STRINGID_STATROSE;
...
static u8 ItemEffectMoveEnd(u32 battler, u16 holdEffect)
{
    ...
    case HOLD_EFFECT_BERSERK_GENE:
        ...
        {
            gStatuses4[gEffectBattler] |= STATUS2_CONFUSION;
        }
        SET_STATCHANGER(STAT_ATK, 1, FALSE);
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_WRING_OUT:
        if(move == MOVE_HARD_PRESS)
            basePower = 140 * gBattleMons[battlerDef].hp / gBattleMons[battlerDef].maxHP;
        else
    ...
    case EFFECT_RAGE_FIST:
        ...
        basePower = (basePower > 1000) ? 1000 : basePower;
        break;
    case EFFECT_FICKLE_BEAM:
        if (RandomPercentage(RNG_FICKLE_BEAM, 50))
            basePower *= 2;
        break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_RIVALRY:
        ...
        break;
    case ABILITY_TRANSPHOBIA:
        if (GetBattlerGender(battlerDef) == MON_GENDERLESS)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    case ABILITY_HOMOPHOBIA:
        if(AreBattlersOfSameGender(battlerDef, BATTLE_PARTNER(battlerDef)))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    case ABILITY_ANALYTIC:
        if (GetBattlerTurnOrderNum(battlerAtk) > GetBattlerTurnOrderNum(battlerDef) && move != MOVE_FUTURE_SIGHT && move != MOVE_DOOM_DESIRE)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    if (isCrit && move == MOVE_VISE_GRIP){ //first place i could find to stick the vise grip power boost
...
static inline u32 CalcDefenseStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    case ABILITY_ARCTIC_RUSH:
        if (!usesDefStat && WEATHER_HAS_EFFECT && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW | B_WEATHER_RAIN)))
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
            if (updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_ARCTIC_RUSH);
        }
        break;
    case ABILITY_WATER_PRESSURE:
        if (WEATHER_HAS_EFFECT && (gBattleWeather & (B_WEATHER_RAIN)))
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
            if (updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_WATER_PRESSURE);
        }
        break;
    }
...
uq4_12_t CalcTypeEffectivenessMultiplier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, u32 defAbility, bool32 recordAbilities)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (gBattleMoves[move].effect == EFFECT_TWO_TYPED_MOVE || move == MOVE_FIERY_DANCE || move == MOVE_FREEZING_GLARE || move == MOVE_THUNDEROUS_KICK)
...
static bool32 IsUnnerveAbilityOnOpposingSide(u32 battler)
{
    if (IsAbilityOnOpposingSide(battler, ABILITY_PUTRID_STENCH))
        return TRUE;
    return FALSE;
}
...
bool32 CompareStat(u32 battler, u8 statId, u8 cmpTo, u8 cmpKind)
{
    ...
    if (GetBattlerAbility(battler) == ABILITY_CONTRARY || GetBattlerAbility(battler) == ABILITY_DOUBLE_DOWN)
    {
...
{
    bool32 hasContrary = (GetBattlerAbility(battler) == ABILITY_CONTRARY || GetBattlerAbility(battler) == ABILITY_DOUBLE_DOWN);
...
static void SetRandomMultiHitCounter()
{
    ...
    else if (gCurrentMove == MOVE_TAIL_SLAP)
        gMultiHitCounter = RandomWeighted(RNG_HITS, 0, 0, 0, 1, 1, 1); // Tail Slap has an equal chance of 3, 4 or 5
