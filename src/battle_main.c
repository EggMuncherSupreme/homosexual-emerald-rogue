...
const u8* FaintClearSetData(u32 battler)
{
    ...
    gProtectStructs[battler].stunShielded = FALSE;
                ...
                case B_ACTION_SWITCH:
                    *(gBattleStruct->battlerPartyIndexes + battler) = gBattlerPartyIndexes[battler];
                        ...
                        || !CanBattlerEscape(battler))
                    {
                      ...
                      && (GetBattlerAbility(battler) != ABILITY_RUN_AWAY))   // must be last to keep i value integrity
                    {
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    {
        if ((ability == ABILITY_SWIFT_SWIM || ability == ABILITY_ARCTIC_RUSH)       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            speed *= 2;
        else if (ability == ABILITY_CHLOROPHYLL && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_SUN)
            speed *= 2;
        else if (ability == ABILITY_SAND_RUSH   && gBattleWeather & B_WEATHER_SANDSTORM)
            speed *= 2;
        else if ((ability == ABILITY_SLUSH_RUSH || ability == ABILITY_ARCTIC_RUSH)  && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW)))
            speed *= 2;
    }
                              ...
                              u32 holdEffectBattler1, u32 holdEffectBattler2, u32 speedBattler1, u32 speedBattler2, s32 priority1, s32 priority2)
{
    ...
    if (priority1 == priority2)
    {
        ...
        if ((gBattleResources->flags->flags[battler1] & RESOURCE_FLAG_EMERGENCY_EXIT) && !(gBattleResources->flags->flags[battler2] & RESOURCE_FLAG_EMERGENCY_EXIT))
            ...
            strikesFirst = 0;
        else if (ability1 == ABILITY_MYCELIUM_MIGHT && ability2 != ABILITY_MYCELIUM_MIGHT && gBattleMoves[gChosenMoveByBattler[battler1]].powderMove)
            strikesFirst = 1;
        else if (ability2 == ABILITY_MYCELIUM_MIGHT && ability1 != ABILITY_MYCELIUM_MIGHT && gBattleMoves[gChosenMoveByBattler[battler2]].powderMove)
            strikesFirst = 0;
        else
        {
...
static void TryChangingTurnOrderEffects(u32 battler1, u32 battler2)
{
    ...
    || (ability1 == ABILITY_QUICK_DRAW && !IS_MOVE_STATUS(gChosenMoveByBattler[battler1]) && gBattleMoves[gChosenMoveByBattler[battler1]].ballisticMove && RandomPercentage(RNG_QUICK_DRAW, 60)))
    ...
    || (ability2 == ABILITY_QUICK_DRAW && !IS_MOVE_STATUS(gChosenMoveByBattler[battler2]) && gBattleMoves[gChosenMoveByBattler[battler2]].ballisticMove && RandomPercentage(RNG_QUICK_DRAW, 60)))
